...
%---------------------------------------------------------
  \cventry
    ...
    {Sept. 2025 - Current} % Date(s)
    {
      ...
      \vspace{8pt}
        \item {Owned the Mastercard Crypto Credentials frontend, including wallet linking, message/transaction signing, and error-handling flows across partner wallets. Led migration from Web3Modal to Reown AppKit.}
        \item {Delivered end-to-end frontend flows for Spend Card and Ledger Spend users: eligibility checks, KYC handoff, card issuance, top-ups, limits, transaction history, balance states, and UI mocks.}
        \item {Built and maintained partner wallet integrations (Ledger, MetaMask, Trust Wallet, and others) in a React + Redux widget with white-label embedding, theming, feature flags, localization, and iframe-host communication.}
        \item {Expanded automated test coverage for high-risk payment flows using Playwright E2E and Vitest unit tests (on/off-ramp, KYC redirects, approvals/signatures, and card onboarding).}
        \item {Integrated core Web3 UX primitives: WalletConnect/Reown AppKit connection, multi-chain EVM support, message/transaction signing, and resilient retry/error states across wallets.}
        \item {Improved production diagnostics by instrumenting client-side observability with Sentry and product event analytics.}
        \item {Built AI-assisted engineering workflows by defining repository agent rules and reusable skills for code generation, test authoring, and review checklists.}
      \end{cvitems}
    }
    {\vspace{24pt}}
...
%---------------------------------------------------------
  \cventry
    ...
    {Aug. 2025 - Mar. 2026} % Date(s)
    {
      ...
      \vspace{8pt}
       \item {Designed architecture and built a Next.js + TypeScript frontend dApp from scratch, including wallet connection, on-chain interactions, and user flows for lenders and borrowers. Integrated Sentry, logging, and event analytics to monitor the product.}
 ...
 \cventry
    ...
    {Sept. 2020 - Mar. 2021} % Date(s)
    {
      ...
      \vspace{8pt}
        ...
        \item {Worked on an analytics React application with a Python backend,
