...
%---------------------------------------------------------
  \cventry
    ...
    {Sent. 2025 - Current} % Date(s)
    {
      ...
      \vspace{8pt}
      \item[] {\small\textit{Mercuryo is one of the leading companies in crypto-powered payments infrastructure, providing a wallet-integrated on/off-ramp widget, Spend Card, and Wallet that unify fiat and crypto rails for apps and exchanges worldwide.}}
      \vspace{8pt}
        \item {Led the transition of the Mercuryo React+Redux widget to \texttt{@reown/appkit} to support Mastercard Crypto Credentials, enabling smoother wallet connection, messages/trxs signing, and credential-based user flows for Mastercard users.}
        \item {Owned end-to-end frontend delivery for Crypto/Spend Card users (eligibility, onboarding, KYC handoff, card issuance, top-ups, limits, transaction history, balance, visuals).}
        \item {Shipped and maintained multiple partner wallet integrations (Ledger, Metamask, TrustWallet and others) of the widget, including white-label embedding, theming, config-driven feature flags, localization, iframes messages communication.}
        \item {Implemented Playwright E2E coverage for high-risk flows (on/off-ramp, KYC redirects, approvals/signatures, card onboarding).}
        \item {Integrated complex web3 flows into UX: WalletConnect/Reown connection, EVM chain switching, message/transaction signing, safe error/retry states across wallets.}
        \item {Improved reliability and debuggability of production issues by adding client-side observability with Sentry.}
      \end{cvitems}
    }
    {\vspace{16pt}}
